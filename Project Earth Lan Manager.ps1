...
$script:PelNetworkSecret = "nPKRhk8X2AUkfTN8"
...
$script:PelGameDetectScript = {
    ...
    foreach ($p in $procs) {
        ...
        # Titel, der nur der EXE-Name ist (z. B. Verknüpfung "FEARMP" aus der Game-Suche),
        # durch einen lesbaren Namen ersetzen
        $exeTitle = (-not $title) -or ($title -ieq $baseName) -or ($title -ieq $exeName)
        if ($KnownExes.ContainsKey($lower)) {
            $k = $KnownExes[$lower]
            if ($exeTitle -and $k.Title) { $title = [string]$k.Title; $exeTitle = $false }
            $ports = @($k.Ports)
            $probe = [string]$k.Probe
        }
        $nice = @{ 'fear.exe' = 'F.E.A.R.'; 'fearmp.exe' = 'F.E.A.R. Combat'; 'fearserver.exe' = 'F.E.A.R.'; 'fearxp.exe' = 'F.E.A.R. Extraction Point'; 'fearpm.exe' = 'F.E.A.R. Perseus Mandate'; 'ut2004.exe' = 'Unreal Tournament 2004'; 'ucc.exe' = 'Unreal Tournament 2004'; 'srcds.exe' = 'Source Dedicated Server'; 'hl2.exe' = 'Half-Life 2'; 'cstrike.exe' = 'Counter-Strike' }
        # Kurzname ohne Leerzeichen (z. B. Ordner/Verknüpfung "FEARMP") bei bekannten EXEs ersetzen
        if ($title -and $title -notmatch '\s' -and $nice.ContainsKey($lower)) { $exeTitle = $true }
        if ($exeTitle -and $title) {
            if ($nice.ContainsKey($lower)) { $title = $nice[$lower] }
            else {
                try {
                    $vi = [System.Diagnostics.FileVersionInfo]::GetVersionInfo($path)
                    foreach ($cand in @([string]$vi.ProductName, [string]$vi.FileDescription)) {
                        $cand = $cand.Trim()
                        if ($cand.Length -ge 3 -and $cand -notmatch '(?i)^(game|launcher|application|client|server|microsoft|windows|unknown)$' -and $cand -notmatch '(?i)\.exe$') { $title = $cand; break }
                    }
                } catch { }
            }
        }
        if (-not $title) { continue }
        # Server-Prozesse als solche kennzeichnen (Dedicated Server, -dedicated usw.)
        $isServer = ($lower -match '(?i)server|dedicated|^srcds|^ucc\.exe$|^hlds') -or ([string]$p.CommandLine -match '(?i)(^|\s)[-+]dedicated\b|\s-server\b')
        if ($isServer -and $title -notmatch '(?i)server') { $title = $title + ' (Server)' }
    ...
    try {
        ...
        try {
            if (-not (Get-NetFirewallRule -DisplayName "Project Earth LAN Postfach $($script:PelMailPort) (TCP)" -ErrorAction SilentlyContinue)) {
                New-NetFirewallRule -DisplayName "Project Earth LAN Postfach $($script:PelMailPort) (TCP)" -Direction Inbound -Action Allow -Protocol TCP -LocalPort $script:PelMailPort -RemoteAddress Any -Profile Any -ErrorAction Stop | Out-Null
            }
        } catch { }
...
# ------------------------------------------------------------------------------
# ------------------------------------------------------------------------------
# OPTION 1 (Erweiterung): Silent-/Unattended-Installation zusätzlicher VPN- und
# LAN-Tools (Radmin VPN, Tailscale, WireGuard, LANemu). Läuft direkt im Anschluss
# an die ZeroTier-Einrichtung, jedes Tool in einem eigenen try/catch, damit ein
# fehlgeschlagenes Tool weder die übrigen Tools noch die ZeroTier-Installation
# abbricht. Ergebnis (OK/Fehler je Tool) wird gesammelt und am Ende angezeigt.
#
# HINWEIS ZU DEN SILENT-SCHALTERN: Die unten verwendeten Flags (--mode unattended
# für Radmin VPN, /quiet für Tailscale/WireGuard) entsprechen dem aktuell
# dokumentierten bzw. für den jeweiligen Installer-Typ (BitRock/InstallBuilder
# bzw. WiX-Bootstrapper/MSI) üblichen Verhalten. Hersteller ändern Installer
# gelegentlich ohne Vorankündigung - schlägt eine Silent-Installation fehl
# (ExitCode <> 0), wird das im Ergebnis-Log unten sichtbar, und der Installer
# kann bei Bedarf manuell aus %TEMP% nachvollzogen werden.
# ------------------------------------------------------------------------------
$script:PelToolInstallLog = [System.Collections.Generic.List[string]]::new()

function Write-PelToolInstallResult([string]$Tool, [bool]$Ok, [string]$Detail = '') {
    if ($Ok) {
        $line = "[OK]      $Tool" + $(if ($Detail) { " ($Detail)" } else { "" })
    } else {
        $line = "[FEHLER]  $Tool - $Detail"
    }
    $script:PelToolInstallLog.Add($line)
}

# Lädt eine Datei mit TLS1.2 und bis zu $Retries Wiederholungen; prüft danach,
# dass die Datei existiert und eine plausible Mindestgröße hat (fängt kaputte/
# leere Downloads ab, die sonst als "Installer gestartet, aber nichts passiert"
# auffallen würden).
function Get-PelDownloadFile {
    param([string]$Url, [string]$OutFile, [int]$Retries = 2, [int]$MinBytes = 10000)
    [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
    $attempt = 0
    $lastErr = "Unbekannter Fehler."
    while ($attempt -le $Retries) {
        try {
            ...
            $lastErr = "Datei fehlt oder ist kleiner als $MinBytes Bytes nach dem Download."
        } catch {
...
function Install-PelRadminVpn {
    param([scriptblock]$Report, [int]$From, [int]$To)
    &$Report "Radmin VPN: Prüfe vorhandene Installation..." $From
    $already = (Test-Path "$env:ProgramFiles(x86)\Radmin VPN\RvSrvHelper.exe") -or (Test-Path "$env:ProgramFiles\Radmin VPN\RvSrvHelper.exe")
    if ($already) {
        Write-PelToolInstallResult "Radmin VPN" $true "bereits installiert"
        &$Report "Radmin VPN bereits vorhanden - übersprungen." $To
        return
    }
    $out = "$env:TEMP\RadminVPN_Setup.exe"
    try {
        &$Report "Radmin VPN: Download..." $From
        Get-PelDownloadFile -Url "https://www.radmin-vpn.com/media/rvpn/RadminVPN_Setup.exe" -OutFile $out
        &$Report "Radmin VPN: Silent-Installation..." ([int](($From + $To) / 2))
        # Radmin VPN basiert auf einem InstallBuilder/BitRock-Installer -> unattended
        # Modus über --mode unattended --unattendedmodeui none.
        $p = Start-Process -FilePath $out -ArgumentList '--mode unattended --unattendedmodeui none' -Wait -PassThru -ErrorAction Stop
        if ($p.ExitCode -ne 0) { throw "Installer-Exit-Code $($p.ExitCode)" }
        Write-PelToolInstallResult "Radmin VPN" $true
    } catch {
        Write-PelToolInstallResult "Radmin VPN" $false $_.Exception.Message
    } finally {
        Remove-Item -Path $out -Force -ErrorAction SilentlyContinue
    }
    &$Report "Radmin VPN abgeschlossen." $To
}

function Install-PelTailscale {
    param([scriptblock]$Report, [int]$From, [int]$To)
    &$Report "Tailscale: Prüfe vorhandene Installation..." $From
    if (Test-Path "$env:ProgramFiles\Tailscale\tailscale.exe") {
        Write-PelToolInstallResult "Tailscale" $true "bereits installiert"
        &$Report "Tailscale bereits vorhanden - übersprungen." $To
        return
    }
    $out = "$env:TEMP\tailscale-setup-latest.exe"
    try {
        &$Report "Tailscale: Download..." $From
        Get-PelDownloadFile -Url "https://pkgs.tailscale.com/stable/tailscale-setup-latest.exe" -OutFile $out
        &$Report "Tailscale: Silent-Installation..." ([int](($From + $To) / 2))
        # Offizieller Windows-Installer (MSI-Bootstrapper) unterstützt /quiet für eine
        # vollständig unbeaufsichtigte Installation inkl. Tailscale-Dienst.
        $p = Start-Process -FilePath $out -ArgumentList '/quiet' -Wait -PassThru -ErrorAction Stop
        if ($p.ExitCode -ne 0) { throw "Installer-Exit-Code $($p.ExitCode)" }
        Write-PelToolInstallResult "Tailscale" $true
    } catch {
        Write-PelToolInstallResult "Tailscale" $false $_.Exception.Message
    } finally {
        Remove-Item -Path $out -Force -ErrorAction SilentlyContinue
    }
    &$Report "Tailscale abgeschlossen." $To
}

function Install-PelWireGuard {
    param([scriptblock]$Report, [int]$From, [int]$To)
    &$Report "WireGuard: Prüfe vorhandene Installation..." $From
    if (Test-Path "$env:ProgramFiles\WireGuard\wireguard.exe") {
        Write-PelToolInstallResult "WireGuard" $true "bereits installiert"
        &$Report "WireGuard bereits vorhanden - übersprungen." $To
        return
    }
    $url = if ([Environment]::Is64BitOperatingSystem) {
        "https://download.wireguard.com/windows-client/wireguard-installer-amd64.exe"
    } else {
        "https://download.wireguard.com/windows-client/wireguard-installer-x86.exe"
    }
    $out = "$env:TEMP\wireguard-installer.exe"
    try {
        &$Report "WireGuard: Download..." $From
        Get-PelDownloadFile -Url $url -OutFile $out
        &$Report "WireGuard: Silent-Installation..." ([int](($From + $To) / 2))
        # WireGuard-Installer ist ein WiX-Burn-Bootstrapper -> Standard-Silent-
        # Schalter /quiet installiert Client + Tunnel-Dienst ohne UI.
        $p = Start-Process -FilePath $out -ArgumentList '/quiet' -Wait -PassThru -ErrorAction Stop
        if ($p.ExitCode -ne 0) { throw "Installer-Exit-Code $($p.ExitCode)" }
        Write-PelToolInstallResult "WireGuard" $true
    } catch {
        Write-PelToolInstallResult "WireGuard" $false $_.Exception.Message
    } finally {
        Remove-Item -Path $out -Force -ErrorAction SilentlyContinue
    }
    &$Report "WireGuard abgeschlossen." $To
}

function Install-PelLanemu {
    param([scriptblock]$Report, [int]$From, [int]$To)
    &$Report "LANemu: Prüfe vorhandene Installation..." $From
    $installDir = Join-Path $env:ProgramFiles "LANemu"
    if (Test-Path (Join-Path $installDir "LANemu.exe")) {
        Write-PelToolInstallResult "LANemu" $true "bereits installiert"
        &$Report "LANemu bereits vorhanden - übersprungen." $To
        return
    }
    try {
        &$Report "LANemu: Suche neueste Version (GitLab)..." $From
        # LANemu hat keinen klassischen Installer, sondern wird als GitLab-Release
        # veröffentlicht. BUGFIX: Vorher wurde der URL-kodierte Projektpfad
        # "Monsterovich%2Flanemu" verwendet - .NET/PowerShell entschlüsselt %2F beim
        # Versenden der Anfrage teils fälschlich wieder zu einem echten "/", wodurch
        # GitLab die Projekt-URL nicht mehr auflösen konnte (404 "Nicht gefunden",
        # wie tatsächlich beobachtet). Fix: die numerische Projekt-ID verwenden
        # (48385306, abgelesen aus dem öffentlichen Avatar-Pfad des Projekts) - die
        # ist eindeutig und umgeht das Encoding-Problem vollständig.
        [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
        $apiUrl = "https://gitlab.com/api/v4/projects/48385306/releases"
        $releases = Invoke-RestMethod -Uri $apiUrl -UseBasicParsing -TimeoutSec 60
        if (-not $releases -or @($releases).Count -eq 0) { throw "Keine Releases über die GitLab-API gefunden." }
        $allAssets = @(@($releases[0].assets.links) | Where-Object { $_.url -match '\.(zip|exe)$' })
        if ($allAssets.Count -eq 0) { throw "Im neuesten Release wurde kein .zip/.exe-Asset gefunden." }
        # Mehrere Betriebssystem-Pakete pro Release (Windows/Linux/macOS/FreeBSD) ->
        # gezielt das Windows-Paket bevorzugen, sonst irrtümlich z. B. das Linux-Zip laden.
        $asset = $allAssets | Where-Object { $_.name -match 'win' -or $_.url -match 'win' } | Select-Object -First 1
        if (-not $asset) { $asset = $allAssets | Select-Object -First 1 }
        $ext = [System.IO.Path]::GetExtension($asset.url)
        $out = "$env:TEMP\LANemu_download$ext"
        &$Report "LANemu: Download..." ([int]($From + ($To - $From) * 0.4))
        Get-PelDownloadFile -Url $asset.url -OutFile $out
        &$Report "LANemu: Installation..." ([int](($From + $To) / 2))
        New-Item -Path $installDir -ItemType Directory -Force | Out-Null
        if ($ext -eq '.zip') {
            Expand-Archive -Path $out -DestinationPath $installDir -Force
        } else {
            Copy-Item -Path $out -Destination (Join-Path $installDir "LANemu.exe") -Force
        }
        Remove-Item -Path $out -Force -ErrorAction SilentlyContinue
        Write-PelToolInstallResult "LANemu" $true
    } catch {
        Write-PelToolInstallResult "LANemu" $false ($_.Exception.Message + " (Alternative: manuell laden über https://gitlab.com/Monsterovich/lanemu/-/releases/permalink/latest)")
    }
    &$Report "LANemu abgeschlossen." $To
}

function Install-PelHyperNat {
    param([scriptblock]$Report, [int]$From, [int]$To)
    &$Report "HyperNAT: Prüfe vorhandene Installation..." $From
    # HyperNAT wird laut Anleitung direkt in den LANemu-Ordner entpackt (dort erwartet
    # es z. B. LANemu diese Anwendung als NAT-Traversal-Helfer bei Symmetric NAT).
    # Existiert der LANemu-Ordner bereits, landet HyperNAT dort - sonst in einem
    # eigenen Ordner unter Program Files.
    $lanemuDir = Join-Path $env:ProgramFiles "LANemu"
    $installDir = if (Test-Path $lanemuDir) { $lanemuDir } else { Join-Path $env:ProgramFiles "HyperNAT" }
    if (Test-Path (Join-Path $installDir "hyper-nat.exe")) {
        Write-PelToolInstallResult "HyperNAT" $true "bereits installiert"
        &$Report "HyperNAT bereits vorhanden - übersprungen." $To
        return
    }
    $out = "$env:TEMP\forwarder.zip"
    try {
        &$Report "HyperNAT: Download..." $From
        [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
        # GitHub bietet für Assets eines Releases einen stabilen Permalink, der immer
        # auf das aktuellste Release zeigt (kein API-Aufruf/JSON-Parsing nötig):
        Get-PelDownloadFile -Url "https://github.com/AnEntrypoint/hyper-nat/releases/latest/download/forwarder.zip" -OutFile $out
        &$Report "HyperNAT: Installation..." ([int](($From + $To) / 2))
        New-Item -Path $installDir -ItemType Directory -Force | Out-Null
        Expand-Archive -Path $out -DestinationPath $installDir -Force
        Remove-Item -Path $out -Force -ErrorAction SilentlyContinue
        Write-PelToolInstallResult "HyperNAT" $true "in $installDir"
    } catch {
        Write-PelToolInstallResult "HyperNAT" $false ($_.Exception.Message + " (Alternative: manuell laden über https://github.com/AnEntrypoint/hyper-nat/releases/latest)")
    }
    &$Report "HyperNAT abgeschlossen." $To
}

function Invoke-InstallZeroTier {
    $script:PelToolInstallLog = [System.Collections.Generic.List[string]]::new()
    Show-ProgressDialog -Title "Installation: ZeroTier One (Project Earth LAN)" -TaskScript {
        param($report)
        ...
        if ($installProcess.ExitCode -ne 0) {
            ...
            [System.Windows.Forms.MessageBox]::Show("ZeroTier One konnte nicht installiert werden (MSI-Exit-Code $($installProcess.ExitCode)).", "Installation fehlgeschlagen", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error) | Out-Null
            return
        }
...
"@
        ...
        &$report "ZeroTier One erfolgreich eingerichtet!" 100
    }
}

# ------------------------------------------------------------------------------
# Einzel-Installer als eigenständige Menüpunkte im Auswahlfenster (siehe
# Show-PelVpnInstallChooser weiter unten): jeder öffnet seinen eigenen
# Fortschrittsdialog (0-100%) und zeigt am Ende ein kurzes Ergebnis-Popup, statt
# wie zuvor automatisch alle vier Tools nacheinander durchzulaufen.
# ------------------------------------------------------------------------------
function Invoke-InstallRadminVpn {
    Show-ProgressDialog -Title "Installation: Radmin VPN" -TaskScript {
        param($report)
        $script:PelToolInstallLog = [System.Collections.Generic.List[string]]::new()
        Install-PelRadminVpn -Report $report -From 5 -To 95
        &$report "Fertig." 100
        [System.Windows.Forms.MessageBox]::Show(($script:PelToolInstallLog -join "`r`n"), "Radmin VPN", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Information) | Out-Null
    }
}

function Invoke-InstallTailscale {
    Show-ProgressDialog -Title "Installation: Tailscale" -TaskScript {
        param($report)
        $script:PelToolInstallLog = [System.Collections.Generic.List[string]]::new()
        Install-PelTailscale -Report $report -From 5 -To 95
        &$report "Fertig." 100
        [System.Windows.Forms.MessageBox]::Show(($script:PelToolInstallLog -join "`r`n"), "Tailscale", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Information) | Out-Null
    }
}

function Invoke-InstallWireGuard {
    Show-ProgressDialog -Title "Installation: WireGuard" -TaskScript {
        param($report)
        $script:PelToolInstallLog = [System.Collections.Generic.List[string]]::new()
        Install-PelWireGuard -Report $report -From 5 -To 95
        &$report "Fertig." 100
        [System.Windows.Forms.MessageBox]::Show(($script:PelToolInstallLog -join "`r`n"), "WireGuard", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Information) | Out-Null
    }
}

function Invoke-InstallLanemu {
    Show-ProgressDialog -Title "Installation: LANemu" -TaskScript {
        param($report)
        $script:PelToolInstallLog = [System.Collections.Generic.List[string]]::new()
        Install-PelLanemu -Report $report -From 5 -To 95
        &$report "Fertig." 100
        [System.Windows.Forms.MessageBox]::Show(($script:PelToolInstallLog -join "`r`n"), "LANemu", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Information) | Out-Null
    }
}

function Invoke-InstallHyperNat {
    Show-ProgressDialog -Title "Installation: HyperNAT" -TaskScript {
        param($report)
        $script:PelToolInstallLog = [System.Collections.Generic.List[string]]::new()
        Install-PelHyperNat -Report $report -From 5 -To 95
        &$report "Fertig." 100
        [System.Windows.Forms.MessageBox]::Show(($script:PelToolInstallLog -join "`r`n"), "HyperNAT", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Information) | Out-Null
    }
}

# ------------------------------------------------------------------------------
# OPTION 1: Auswahlfenster "Welchen VPN-/LAN-Dienst installieren?"
# Ersetzt die bisherige automatische Installation aller Tools durch ein Fenster
# mit einem Button je Dienst - der Nutzer entscheidet, was heruntergeladen und
# benutzt werden soll. Jeder Button installiert nur seinen eigenen Dienst
# (silent/unattended im Hintergrund) und kann beliebig oft/mehrfach genutzt
# werden, ohne das Fenster neu zu öffnen.
# ------------------------------------------------------------------------------
# ------------------------------------------------------------------------------
# Alle Ports des Managers auf einmal öffnen/schließen (Windows-Firewall-Regeln).
# Nützlich z. B. wenn jemand die Firewall-Abfrage beim ersten Start versehentlich
# weggeklickt hat, oder um vor einem LAN-Abend testweise alles freizugeben bzw.
# danach wieder komplett zu schließen. Wirkt unabhängig davon, ob der jeweilige
# Dienst (Chat/Datei/Voice/...) gerade läuft - die Liste ist bewusst fest
# hinterlegt, damit "Alle Ports" wirklich immer allen genutzten Ports entspricht.
# ------------------------------------------------------------------------------
function Get-PelAllPortRules {
    return @(
        @{ Name = "Project Earth LAN Postfach $($script:PelMailPort) (TCP)"; Proto = 'TCP'; Port = $script:PelMailPort }
        @{ Name = "Project Earth LAN Status $($script:PelStatusPort) (UDP)"; Proto = 'UDP'; Port = $script:PelStatusPort }
...
function Enable-PelAllPorts {
    param([scriptblock]$Report = $null)
    $rules = @(Get-PelAllPortRules)
    $ok = 0; $fail = 0; $i = 0
    foreach ($r in $rules) {
        $i++
        try {
            if (-not (Get-NetFirewallRule -DisplayName $r.Name -ErrorAction SilentlyContinue)) {
                New-NetFirewallRule -DisplayName $r.Name -Direction Inbound -Action Allow -Protocol $r.Proto -LocalPort $r.Port -RemoteAddress Any -Profile Any -ErrorAction Stop | Out-Null
            }
            $ok++
        } catch { $fail++ }
        if ($Report) { &$Report "Öffne: $($r.Name)" ([int]($i * 100 / $rules.Count)) }
    }
    return [pscustomobject]@{ Ok = $ok; Fail = $fail; Total = $rules.Count }
}

function Disable-PelAllPorts {
    param([scriptblock]$Report = $null)
    $rules = @(Get-PelAllPortRules)
    $ok = 0; $fail = 0; $i = 0
    foreach ($r in $rules) {
        ...
        if ($Report) { &$Report "Schließe: $($r.Name)" ([int]($i * 100 / $rules.Count)) }
    }
    return [pscustomobject]@{ Ok = $ok; Fail = $fail; Total = $rules.Count }
}

function Invoke-PelEnableAllPorts {
    Show-ProgressDialog -Title "Alle Manager-Ports öffnen" -TaskScript {
        param($report)
        $res = Enable-PelAllPorts -Report $report
        &$report "Fertig." 100
        [System.Windows.Forms.MessageBox]::Show("Alle Ports des Managers wurden in der Windows-Firewall freigegeben ($($res.Ok) von $($res.Total)).", "Ports geöffnet", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Information) | Out-Null
    }
}

function Invoke-PelDisableAllPorts {
    $confirm = [System.Windows.Forms.MessageBox]::Show("Wirklich alle Ports des Managers in der Windows-Firewall schließen? Chat, Datei-Transfer, Voice und Server-Suche funktionieren danach nicht mehr, bis die Ports wieder geöffnet werden.", "Alle Ports schließen", [System.Windows.Forms.MessageBoxButtons]::YesNo, [System.Windows.Forms.MessageBoxIcon]::Warning)
    if ($confirm -ne [System.Windows.Forms.DialogResult]::Yes) { return }
    Show-ProgressDialog -Title "Alle Manager-Ports schließen" -TaskScript {
        param($report)
        $res = Disable-PelAllPorts -Report $report
        &$report "Fertig." 100
        [System.Windows.Forms.MessageBox]::Show("Alle Ports des Managers wurden in der Windows-Firewall geschlossen ($($res.Ok) von $($res.Total)).", "Ports geschlossen", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Information) | Out-Null
    }
}

function Show-PelVpnInstallChooser {
    $dlg = New-Object System.Windows.Forms.Form
    $dlg.Text = "VPN- / LAN-Dienst installieren"
    ...
    $lblHead.Text = "Welchen Dienst möchtest du herunterladen und benutzen?"
    $lblHead.Location = New-Object System.Drawing.Point(20, 15)
    $lblHead.Size = New-Object System.Drawing.Size(430, 40)
    $lblHead.Font = $fontHead
    $lblHead.ForeColor = [System.Drawing.Color]::White
    $dlg.Controls.Add($lblHead)

    # Eintrag = Button-Text, Untertitel-Text, aufzurufende Installationsfunktion
    $entries = @(
        @{ Title = "ZeroTier One"; Sub = "Standard für Project Earth LAN - empfohlen, wenn du unsicher bist."; Action = { Invoke-InstallZeroTier } },
        @{ Title = "Radmin VPN"; Sub = "Einfache Alternative, gut für kleine Gruppen."; Action = { Invoke-InstallRadminVpn } },
        @{ Title = "Tailscale"; Sub = "Moderne VPN-Lösung, benötigt einen kostenlosen Account."; Action = { Invoke-InstallTailscale } },
        @{ Title = "WireGuard"; Sub = "Schnelle, schlanke VPN-Software (Konfiguration von Hand nötig)."; Action = { Invoke-InstallWireGuard } },
        @{ Title = "LANemu"; Sub = "Simuliert ein echtes LAN-Netzwerk für ältere Spiele."; Action = { Invoke-InstallLanemu } },
        @{ Title = "HyperNAT"; Sub = "NAT-Traversal-Helfer für LANemu bei Symmetric NAT (kein VPN für sich allein)."; Action = { Invoke-InstallHyperNat } }
    )
    ...
    foreach ($e in $entries) {
        ...
        $btn.Size = New-Object System.Drawing.Size(430, 40)
        $btn.BackColor = [System.Drawing.Color]::FromArgb(0, 120, 215)
        $btn.ForeColor = [System.Drawing.Color]::White
        $btn.FlatStyle = "Flat"
        $btn.Font = $fontBtn
        $btn.Tag = $e.Action
        $btn.Add_Click({ & $this.Tag }.GetNewClosure())
        $dlg.Controls.Add($btn)

        $lblSub = New-Object System.Windows.Forms.Label
        $lblSub.Text = $e.Sub
        $lblSub.Location = New-Object System.Drawing.Point(24, ($y + 42))
        $lblSub.Size = New-Object System.Drawing.Size(420, 18)
        $lblSub.Font = $fontSub
    ...
    $lblPorts.Text = "Alle vom Manager genutzten Ports (Chat/Datei/Voice/Postfach/Server-Suche) auf einmal in der Windows-Firewall freigeben oder wieder sperren:"
    $lblPorts.Location = New-Object System.Drawing.Point(20, $y)
    $lblPorts.Size = New-Object System.Drawing.Size(430, 34)
    $lblPorts.Font = $fontSub
    $lblPorts.ForeColor = [System.Drawing.Color]::LightGray
    $dlg.Controls.Add($lblPorts)
    $y += 38

    $btnPortsOpen = New-Object System.Windows.Forms.Button
    $btnPortsOpen.Text = "Alle Ports öffnen"
    $btnPortsOpen.Location = New-Object System.Drawing.Point(20, $y)
    $btnPortsOpen.Size = New-Object System.Drawing.Size(208, 38)
    $btnPortsOpen.BackColor = [System.Drawing.Color]::FromArgb(0, 130, 60)
    $btnPortsOpen.ForeColor = [System.Drawing.Color]::White
    $btnPortsOpen.FlatStyle = "Flat"
    $btnPortsOpen.Font = $fontBtn
    $btnPortsOpen.Add_Click({ Invoke-PelEnableAllPorts })
    $dlg.Controls.Add($btnPortsOpen)

    $btnPortsClose = New-Object System.Windows.Forms.Button
    $btnPortsClose.Text = "Alle Ports schließen"
    $btnPortsClose.Location = New-Object System.Drawing.Point(242, $y)
    $btnPortsClose.Size = New-Object System.Drawing.Size(208, 38)
    $btnPortsClose.BackColor = [System.Drawing.Color]::FromArgb(150, 30, 30)
    $btnPortsClose.ForeColor = [System.Drawing.Color]::White
    $btnPortsClose.FlatStyle = "Flat"
    $btnPortsClose.Font = $fontBtn
    $btnPortsClose.Add_Click({ Invoke-PelDisableAllPorts })
    $dlg.Controls.Add($btnPortsClose)
    $y += 50

    $btnClose = New-Object System.Windows.Forms.Button
    $btnClose.Text = "Schließen"
    $btnClose.Location = New-Object System.Drawing.Point(175, $y)
    $btnClose.Size = New-Object System.Drawing.Size(130, 34)
    $btnClose.BackColor = [System.Drawing.Color]::FromArgb(45, 45, 45)
    $btnClose.ForeColor = [System.Drawing.Color]::White
    $btnClose.FlatStyle = "Flat"
...
OPTION 13 - Eigene Netzwerke erstellen & verwalten (ZTNET-Konto)
  Verbindet den Manager mit dem ZTNET-Webpanel (http://10.147.0.34:3000,
  erreichbar über das Netzwerk 091f0945fc744570 aus Option 1).
  - Button 1 öffnet die Registrierung im Webpanel: dort legst du dein
    kostenloses Konto an.
  - Danach erstellst du im Panel (Einstellungen -> Account) deinen eigenen
    API-Token, fügst ihn im Manager ein und klickst auf "Login". Der Token
    wird nur auf deinem PC gespeichert, mit Windows verschlüsselt (DPAPI).
  - Button 2 zeigt deine Netzwerke. Ein Klick öffnet die Verwaltung:
    Netzwerk-ID, IP-Bereich, Mitgliederliste mit Freischalten, Sperren,
    Umbenennen, Beschreibung und Entfernen, dazu "Mit diesem PC beitreten".
  - "Netzwerk entfernen" (nach Rückfrage): Die ZTNET-API 0.8.3 kann Netzwerke
    noch nicht löschen, deshalb öffnet der Manager dafür direkt die passende
    Seite im Webpanel. IP-Bereiche, Routen und Flow Rules stellst du ebenfalls
    dort ein.
  Im Manager steckt kein Admin-Zugang: jeder sieht nur seine eigenen Netzwerke.
...
C:\Project-Earth-Lan\   (gemeinsame Einstellungen aller Fenster)
  ...
  ztnet_token.dat          Dein ZTNET-API-Token (Option 13), mit Windows
                           verschlüsselt - nur dein Benutzerkonto kann ihn lesen.
  ztnet_panel.txt          Adresse des ZTNET-Webpanels (Option 13).
...
'@
    ...
    function Set-LanFirewallRule([bool]$enable) {
        try {
            ...
            if ($enable) {
                New-NetFirewallRule -DisplayName $fwRuleName -Direction Inbound -Action Allow -Protocol TCP -LocalPort 9872 -RemoteAddress Any -Profile Any -ErrorAction Stop | Out-Null
                New-NetFirewallRule -DisplayName "$fwRuleName (UDP)" -Direction Inbound -Action Allow -Protocol UDP -LocalPort 9872 -RemoteAddress Any -Profile Any -ErrorAction Stop | Out-Null
                $state.FwRule = $true
            } else {
                $state.FwRule = $false
            }
    ...
    function Add-FirewallAllow([string]$name, [string]$program, [string[]]$ports) {
        try {
            ...
            if ($program) {
                New-NetFirewallRule -DisplayName $name -Direction Inbound -Action Allow -Program $program -Profile Any -RemoteAddress Any -ErrorAction Stop | Out-Null
            } else {
                New-NetFirewallRule -DisplayName "$name (UDP)" -Direction Inbound -Action Allow -Protocol UDP -LocalPort $ports -Profile Any -RemoteAddress Any -ErrorAction Stop | Out-Null
                New-NetFirewallRule -DisplayName "$name (TCP)" -Direction Inbound -Action Allow -Protocol TCP -LocalPort $ports -Profile Any -RemoteAddress Any -ErrorAction Stop | Out-Null
            }
            return $true
        } catch {
...
function Invoke-CommCenter {
    ...
    # WICHTIG (Netzwerk-Fix): Alle Regeln verwenden bewusst -RemoteAddress Any statt
    # des ursprünglichen LocalSubnet. LocalSubnet erlaubt nur Peers, die Windows als im
    # "gleichen Subnetz" wie ein lokaler Adapter erkennt - das gilt zuverlässig für
    # ZeroTier (klassisches /24-Subnetz), aber NICHT für WireGuard/Tailscale, die pro
    # Peer meist einzelne /32-Routen ohne gemeinsames Subnetz setzen. Mit LocalSubnet
    # kam dort also nie ein eingehendes Paket durch die Firewall - das war die
    # Hauptursache für die "nur ZeroTier funktioniert"-Fixierung.
    function Set-CcFirewall([string]$which, [bool]$enable) {
        try {
            if ($which -eq 'chat') {
                ...
                if ($enable) {
                    New-NetFirewallRule -DisplayName "Project Earth LAN Chat 9874 (TCP)" -Direction Inbound -Action Allow -Protocol TCP -LocalPort 9874 -RemoteAddress Any -Profile Any -ErrorAction Stop | Out-Null
                    New-NetFirewallRule -DisplayName "Project Earth LAN Chat 9874 (UDP)" -Direction Inbound -Action Allow -Protocol UDP -LocalPort 9874 -RemoteAddress Any -Profile Any -ErrorAction Stop | Out-Null
                    $st.ChatFw = $true
                ...
                if ($enable) {
                    New-NetFirewallRule -DisplayName "Project Earth LAN File 9876 (TCP)" -Direction Inbound -Action Allow -Protocol TCP -LocalPort 9876 -RemoteAddress Any -Profile Any -ErrorAction Stop | Out-Null
                    New-NetFirewallRule -DisplayName "Project Earth LAN File 9876 (UDP)" -Direction Inbound -Action Allow -Protocol UDP -LocalPort 9876 -RemoteAddress Any -Profile Any -ErrorAction Stop | Out-Null
                    $st.FileFw = $true
                ...
                if ($enable) {
                    New-NetFirewallRule -DisplayName "Project Earth LAN Voice 9873" -Direction Inbound -Action Allow -Protocol UDP -LocalPort 9873 -RemoteAddress Any -Profile Any -ErrorAction Stop | Out-Null
                    $st.VoiceFw = $true
                ...
                if ($enable) {
                    New-NetFirewallRule -DisplayName "Project Earth LAN Social 9776 (TCP)" -Direction Inbound -Action Allow -Protocol TCP -LocalPort 9776 -RemoteAddress Any -Profile Any -ErrorAction Stop | Out-Null
                    New-NetFirewallRule -DisplayName "Project Earth LAN Social 9776 (UDP)" -Direction Inbound -Action Allow -Protocol UDP -LocalPort 9776 -RemoteAddress Any -Profile Any -ErrorAction Stop | Out-Null
                    $st.SocialFw = $true
    ...
    function Start-CcSocial {
        # BUGFIX (Adapter-Wechsel): vorher gab es hier nur "if ($st.SocialNode) { return }"
        # ohne vorheriges Stop-CcSocial - dadurch behielt der Kanal-Synchronisationsdienst
        # (Port 9776) nach einem Adapterwechsel dauerhaft die alte IP/den alten Socket,
        # weil der Neustart in Start-CcAllNodes (ausgelöst durch die zentrale
        # Adapter-Auswahl) hier sofort wirkungslos abgebrochen wurde. Jetzt wird - wie bei
        # Start-CcChat/Start-CcFile - immer zuerst der alte Socket sauber geschlossen und
        # danach neu auf der aktuellen Adapter-IP gebunden.
        Stop-CcSocial
        if (-not $st.Adapter) { return }
        $a = $st.Adapter
    ...
    function Select-CcDevices {
        ...
        $d = New-Object System.Windows.Forms.Form
        $d.Text = "Mikrofon und Lautsprecher wählen"
        ...
        $d.BackColor = $cBack
        $d.ForeColor = $cWhite
        $l1 = New-CcLabel "Mikrofon [Eingabe] - womit man dich hört:" 14 14 460 22
        ...
        $l2 = New-CcLabel "Lautsprecher [Ausgabe] - womit du die anderen hörst:" 14 80 460 22
    ...
    $btnV5 = New-CcButton "5. Mikrofon / Lautsprecher" $vColX (4 * $vStep) $vColW $vBtnH
...
# ==============================================================================
# OPTION 13 - EIGENE NETZWERKE (ZTNET-KONTO)
# ==============================================================================
# Verbindet den Manager mit der REST-API des ZTNET-Panels (ZeroTier-Weboberfläche,
# getestet gegen ZTNET 0.8.3). Jeder Nutzer meldet sich mit SEINEM persönlichen
# API-Token an (erstellt im Webpanel nach der Account-Erstellung). Im Manager steckt
# bewusst KEIN Admin-Schlüssel - der ließe sich aus der .exe auslesen.
# Der Token wird nur auf diesem PC gespeichert, per Windows-DPAPI verschlüsselt
# (lesbar nur für dein Windows-Benutzerkonto).
#
# Genutzte Endpunkte (Header "x-ztnet-auth", max. 50 Anfragen/Minute):
#   GET    /api/v1/network                           eigene Netzwerke
#   POST   /api/v1/network            {name}         Netzwerk erstellen
#   GET    /api/v1/network/<id>                      Netzwerk-Details
...
# Netzwerk-Einstellungen (IP-Bereiche, Routen, Flow Rules) und das Löschen eines
# Netzwerks bietet die API in 0.8.3 nicht - dafür öffnet der Manager das Webpanel.

$script:PelZtDefaultUrl = 'http://10.147.0.34:3000'
$script:PelZtTokenPath  = 'C:\Project-Earth-Lan\ztnet_token.dat'
$script:PelZtUrlPath    = 'C:\Project-Earth-Lan\ztnet_panel.txt'

function Get-PelZtBaseUrl {
    $u = ''
    try { if (Test-Path -LiteralPath $script:PelZtUrlPath) { $u = ([System.IO.File]::ReadAllText($script:PelZtUrlPath, [System.Text.Encoding]::UTF8)).Trim() } } catch { }
    if ($u -notmatch '^https?://[^\s/]+') { $u = $script:PelZtDefaultUrl }
    return $u.TrimEnd('/')
}

function Save-PelZtBaseUrl([string]$Url) {
    try {
        $dir = Split-Path -Parent $script:PelZtUrlPath
        if (-not (Test-Path -LiteralPath $dir)) { New-Item -Path $dir -ItemType Directory -Force | Out-Null }
        [System.IO.File]::WriteAllText($script:PelZtUrlPath, $Url.Trim().TrimEnd('/'), (New-Object System.Text.UTF8Encoding($false)))
    } catch { }
}

function Get-PelZtToken {
    try {
        if (-not (Test-Path -LiteralPath $script:PelZtTokenPath)) { return '' }
        Add-Type -AssemblyName System.Security
        $enc = [System.IO.File]::ReadAllBytes($script:PelZtTokenPath)
...
function Save-PelZtToken([string]$Token) {
    Add-Type -AssemblyName System.Security
    $dir = Split-Path -Parent $script:PelZtTokenPath
    if (-not (Test-Path -LiteralPath $dir)) { New-Item -Path $dir -ItemType Directory -Force | Out-Null }
    $enc = [System.Security.Cryptography.ProtectedData]::Protect([System.Text.Encoding]::UTF8.GetBytes($Token), $null, [System.Security.Cryptography.DataProtectionScope]::CurrentUser)
    [System.IO.File]::WriteAllBytes($script:PelZtTokenPath, $enc)
}

function Remove-PelZtToken {
    try { if (Test-Path -LiteralPath $script:PelZtTokenPath) { Remove-Item -LiteralPath $script:PelZtTokenPath -Force } } catch { }
}

# Ein API-Aufruf. Rückgabe: @{ Ok; Status; Data; Error }. UTF-8 in beide Richtungen
# (Invoke-RestMethod in PowerShell 5.1 verschluckt sonst Umlaute in Namen).
function Invoke-PelZtApi {
    param([string]$Method, [string]$Path, $Body = $null, [string]$Token = '', [string]$BaseUrl = '')
    $r = @{ Ok = $false; Status = 0; Data = $null; Error = '' }
    if (-not $BaseUrl) { $BaseUrl = Get-PelZtBaseUrl }
    if (-not $Token) { $Token = Get-PelZtToken }
    try {
        ...
        $req.UserAgent = 'ProjectEarthLanManager'
        $req.Headers.Add('x-ztnet-auth', $Token)
        if ($null -ne $Body) {
        ...
        if ($text) { try { $r.Data = $text | ConvertFrom-Json } catch { $r.Data = $text } }
        if ($r.Status -ge 200 -and $r.Status -lt 300) { $r.Ok = $true }
        else {
            $msg = ''
            if ($r.Data -and $r.Data.PSObject.Properties['error']) { $msg = [string]$r.Data.error }
            switch ($r.Status) {
                401 { $r.Error = 'Zugriff verweigert - der API-Token ist ungültig, abgelaufen oder gehört nicht zu diesem Netzwerk.' }
                403 { $r.Error = 'Keine Berechtigung für diese Aktion.' }
                404 { $r.Error = 'Nicht gefunden.' }
                429 { $r.Error = 'Zu viele Anfragen (max. 50 pro Minute) - bitte kurz warten.' }
                default { $r.Error = "Fehler $($r.Status)" }
            }
            if ($msg) { $r.Error += " ($msg)" }
        }
    } catch {
        $r.Error = "Panel nicht erreichbar ($BaseUrl): $($_.Exception.Message)`n`nBist du mit dem Netzwerk 091f0945fc744570 (Account und Netzwerk Verwaltungs Server) verbunden? Das richtet Option 1 ein."
    }
    return $r
}

function Get-PelZtProp($Obj, [string]$Name) {
...
# Zeitangabe aus ZTNET (ISO-Datum oder Millisekunden) als "vor x Min." darstellen
function Format-PelZtSeen($v) {
    if ($null -eq $v -or [string]$v -eq '') { return '-' }
    $dt = $null
    try {
        if ([string]$v -match '^\d{11,}$') { $dt = [DateTimeOffset]::FromUnixTimeMilliseconds([int64]$v).LocalDateTime }
        else { $dt = ([DateTime]$v).ToLocalTime() }
    } catch { return [string]$v }
    $d = (Get-Date) - $dt
    if ($d.TotalMinutes -lt 2) { return 'gerade online' }
    if ($d.TotalMinutes -lt 60) { return ('vor {0} Min.' -f [int]$d.TotalMinutes) }
    if ($d.TotalHours -lt 48) { return ('vor {0} Std.' -f [int]$d.TotalHours) }
    return $dt.ToString('dd.MM.yyyy HH:mm')
}

function Invoke-PelZtJoin([string]$NetId, $Owner) {
    if ($NetId -notmatch '^[0-9a-fA-F]{16}$') { return }
    $cli = Get-ZeroTierCli
    $res = & $cli join $NetId 2>&1
    if ($LASTEXITCODE -eq 0 -or "$res" -like '*200 join OK*') {
        ...
        Show-PelMsg "Dieser PC ist dem Netzwerk $NetId beigetreten.`n`nFalls es privat ist: diesen PC jetzt in der Mitgliederliste freischalten." 'Beitreten' ([System.Windows.Forms.MessageBoxIcon]::Information) $Owner
    } else {
        Show-PelMsg "Beitritt fehlgeschlagen:`n$res" 'Beitreten' ([System.Windows.Forms.MessageBoxIcon]::Error) $Owner
    }
}

# ---- gemeinsame Bausteine für die drei Fenster --------------------------------------
function New-ZtForm([string]$Title, [int]$W, [int]$H) {
    $f = New-Object System.Windows.Forms.Form
    $f.Text = $Title
    $f.Size = New-Object System.Drawing.Size($W, $H)
    $f.MinimumSize = New-Object System.Drawing.Size([Math]::Min($W, 640), [Math]::Min($H, 420))
...
function New-ZtButton([string]$t, [int]$x, [int]$y, [int]$w, [int]$h, [bool]$accent = $false) {
    $b = New-Object System.Windows.Forms.Button
    $b.Text = $t
    $b.Location = New-Object System.Drawing.Point($x, $y)
    $b.Size = New-Object System.Drawing.Size($w, $h)
    ...
    if ($accent) { $b.BackColor = [System.Drawing.Color]::FromArgb(0, 120, 215) } else { $b.BackColor = [System.Drawing.Color]::FromArgb(60, 60, 60) }
    return $b
}
function New-ZtLabel([string]$t, [int]$x, [int]$y, [int]$w, [int]$h) {
    $l = New-Object System.Windows.Forms.Label
    $l.Text = $t
    $l.Location = New-Object System.Drawing.Point($x, $y)
    $l.Size = New-Object System.Drawing.Size($w, $h)
...
function New-ZtText([int]$x, [int]$y, [int]$w) {
    $t = New-Object System.Windows.Forms.TextBox
    $t.Location = New-Object System.Drawing.Point($x, $y)
...
function New-ZtList([int]$x, [int]$y, [int]$w, [int]$h, [string[]]$cols, [int[]]$widths) {
    $lv = New-Object System.Windows.Forms.ListView
    $lv.Location = New-Object System.Drawing.Point($x, $y)
    $lv.Size = New-Object System.Drawing.Size($w, $h)
    $lv.View = 'Details'
...
function Set-ZtAnchor($c, [string[]]$a) {
    $v = 0
    foreach ($x in $a) { $v = $v -bor [int][System.Windows.Forms.AnchorStyles]::$x }
    $c.Anchor = [System.Windows.Forms.AnchorStyles]$v
}
function Read-ZtText([string]$Title, [string]$Prompt, [string]$Default, $Owner) {
    $d = New-ZtForm $Title 460 190
    $d.FormBorderStyle = 'FixedDialog'; $d.MaximizeBox = $false; $d.MinimizeBox = $false
    $d.StartPosition = 'CenterParent'
    $d.Controls.Add((New-ZtLabel $Prompt 14 14 420 22))
    $tb = New-ZtText 14 42 416
    $tb.Text = $Default
    $ok = New-ZtButton 'OK' 214 90 100 32 $true
    $ok.DialogResult = [System.Windows.Forms.DialogResult]::OK
    $cancel = New-ZtButton 'Abbrechen' 320 90 110 32
    $cancel.DialogResult = [System.Windows.Forms.DialogResult]::Cancel
    $d.Controls.AddRange(@($tb, $ok, $cancel))
    $d.AcceptButton = $ok; $d.CancelButton = $cancel
    $res = if ($Owner) { $d.ShowDialog($Owner) } else { $d.ShowDialog() }
    if ($res -eq [System.Windows.Forms.DialogResult]::OK) { return $tb.Text.Trim() }
    return $null
}

# Netzwerk entfernen. Die ZTNET-API (0.8.3, auch aktuelle Versionen) hat dafuer noch
# keinen Endpunkt - der Manager versucht es trotzdem (falls eine spaetere Version ihn
# nachruestet) und oeffnet sonst direkt die Seite im Webpanel, auf der man loescht.
# Rueckgabe: $true, wenn das Netzwerk per API geloescht wurde.
function Remove-PelZtNetworkFlow([string]$NetId, [string]$NetName, $Owner) {
    $anzeige = if ($NetName) { "'$NetName' ($NetId)" } else { $NetId }
    $ans = [System.Windows.Forms.MessageBox]::Show($Owner, "Netzwerk $anzeige wirklich entfernen?`n`nAlle Mitglieder verlieren sofort die Verbindung, die Netzwerk-ID ist danach ungültig. Das lässt sich nicht rückgängig machen.", 'Netzwerk entfernen', [System.Windows.Forms.MessageBoxButtons]::YesNo, [System.Windows.Forms.MessageBoxIcon]::Warning, [System.Windows.Forms.MessageBoxDefaultButton]::Button2)
    if ($ans -ne [System.Windows.Forms.DialogResult]::Yes) { return $false }
    $r = Invoke-PelZtApi -Method 'DELETE' -Path "/api/v1/network/$NetId"
    if ($r.Ok) {
        # Dieser PC verlaesst das geloeschte Netzwerk auch lokal (Adapter wird entfernt)
        try { $cli = Get-ZeroTierCli; [void](& $cli leave $NetId 2>&1) } catch { }
        Show-PelMsg "Netzwerk $anzeige wurde entfernt." 'Netzwerk entfernen' ([System.Windows.Forms.MessageBoxIcon]::Information) $Owner
        return $true
    }
    if ($r.Status -eq 405 -or $r.Status -eq 404) {
        Show-PelMsg ("Deine ZTNET-Version erlaubt das Löschen eines Netzwerks noch nicht über die Schnittstelle - nur im Webpanel.`n`nDas Panel öffnet sich jetzt direkt bei diesem Netzwerk: ganz unten in den Netzwerk-Einstellungen findest du 'Netzwerk löschen'.`n`nDanach hier auf 'Aktualisieren' klicken.") 'Netzwerk entfernen' ([System.Windows.Forms.MessageBoxIcon]::Information) $Owner
        try { Start-Process ((Get-PelZtBaseUrl) + "/network/$NetId") } catch { }
        return $false
    }
    Show-PelMsg "Netzwerk konnte nicht entfernt werden:`n$($r.Error)" 'Netzwerk entfernen' ([System.Windows.Forms.MessageBoxIcon]::Error) $Owner
    return $false
}

# ---- Fenster 3: ein Netzwerk verwalten (Details + Mitglieder) --------------------------
function Show-PelZtNetwork([string]$NetId, [string]$NetName, $Owner) {
    $st = @{ Members = @(); Net = $null; Busy = $false }
    $f = New-ZtForm "Netzwerk verwalten - $NetName ($NetId)" 1000 640

    $lblHead = New-ZtLabel "$NetName" 12 10 700 26
    $lblHead.Font = New-Object System.Drawing.Font('Segoe UI', 12, [System.Drawing.FontStyle]::Bold)
    $lblInfo = New-ZtLabel 'Lade Netzwerk ...' 12 40 960 60
    $lblInfo.ForeColor = [System.Drawing.Color]::LightGray

    $btnCopy = New-ZtButton 'ID kopieren' 12 104 140 32
    $btnJoin = New-ZtButton 'Mit diesem PC beitreten' 160 104 220 32 $true
    $btnWeb  = New-ZtButton 'Einstellungen im Webpanel' 388 104 240 32
    $btnRef  = New-ZtButton 'Aktualisieren' 636 104 150 32
    $btnNetDel = New-ZtButton 'Netzwerk entfernen' 794 104 178 32
    $btnNetDel.BackColor = [System.Drawing.Color]::FromArgb(150, 40, 40)

    $lblMem = New-ZtLabel 'Mitglieder (Doppelklick = umbenennen):' 12 146 500 20
    $lv = New-ZtList 12 168 720 380 @('Name', 'Member-ID', 'Freigeschaltet', 'IP-Adressen', 'Zuletzt gesehen', 'Beschreibung') @(150, 100, 100, 150, 110, 200)

    $colX = 744; $colW = 228; $bh = 34; $step = 40
    $btnAuth   = New-ZtButton 'Freischalten' $colX 168 $colW $bh $true
    $btnDeauth = New-ZtButton 'Sperren' $colX (168 + $step) $colW $bh
    $btnRename = New-ZtButton 'Umbenennen' $colX (168 + 2 * $step) $colW $bh
    $btnDesc   = New-ZtButton 'Beschreibung' $colX (168 + 3 * $step) $colW $bh
    $btnDel    = New-ZtButton 'Aus Netzwerk entfernen' $colX (168 + 4 * $step) $colW $bh
    $btnDel.BackColor = [System.Drawing.Color]::FromArgb(150, 40, 40)
    $lblHint = New-ZtLabel ("Neue Geräte erscheinen hier, sobald sie der Netzwerk-ID beitreten. Bei privaten Netzwerken müssen sie erst freigeschaltet werden.`n`nIP-Bereiche, Routen und Flow Rules stellst du im Webpanel ein - die ZTNET-API bietet dafür (noch) keine Schnittstelle.") $colX (168 + 5 * $step + 6) $colW 170
    $lblHint.ForeColor = [System.Drawing.Color]::LightGray
    $lblStatus = New-ZtLabel '' 12 556 960 22
    $lblStatus.ForeColor = [System.Drawing.Color]::LightGray

    $f.Controls.AddRange(@($lblHead, $lblInfo, $btnCopy, $btnJoin, $btnWeb, $btnRef, $btnNetDel, $lblMem, $lv, $btnAuth, $btnDeauth, $btnRename, $btnDesc, $btnDel, $lblHint, $lblStatus))
    Set-ZtAnchor $lblInfo @('Top', 'Left', 'Right')
    Set-ZtAnchor $lv @('Top', 'Left', 'Right', 'Bottom')
    foreach ($c in @($btnAuth, $btnDeauth, $btnRename, $btnDesc, $btnDel, $lblHint)) { Set-ZtAnchor $c @('Top', 'Right') }
    Set-ZtAnchor $lblStatus @('Left', 'Right', 'Bottom')

    $loadAll = {
        $f.Cursor = [System.Windows.Forms.Cursors]::WaitCursor
        try {
            $rn = Invoke-PelZtApi -Method 'GET' -Path "/api/v1/network/$NetId"
            if ($rn.Ok) {
                $n = $rn.Data
                $st.Net = $n
                $nm = [string](Get-PelZtProp $n 'name'); if ($nm) { $lblHead.Text = $nm }
                $priv = Get-PelZtProp $n 'private'
                $pools = @(Get-PelZtProp $n 'ipAssignmentPools') | Where-Object { $_ } | ForEach-Object { "$($_.ipRangeStart) - $($_.ipRangeEnd)" }
                $routes = @(Get-PelZtProp $n 'routes') | Where-Object { $_ } | ForEach-Object { if ($_.via) { "$($_.target) via $($_.via)" } else { [string]$_.target } }
                $lblInfo.Text = "Netzwerk-ID: $NetId     Zugang: $(if ($priv -eq $false) { 'öffentlich (jeder mit der ID kommt rein)' } else { 'privat (Mitglieder müssen freigeschaltet werden)' })`n" +
                    "IP-Bereich: $(if ($pools) { $pools -join ', ' } else { '(keiner eingestellt)' })`n" +
                    "Routen: $(if ($routes) { $routes -join ', ' } else { '-' })"
            } else { $lblInfo.Text = "Netzwerk-Details nicht abrufbar: $($rn.Error)" }

            $rm = Invoke-PelZtApi -Method 'GET' -Path "/api/v1/network/$NetId/member"
            $sel = if ($lv.SelectedItems.Count -gt 0) { [string]$lv.SelectedItems[0].SubItems[1].Text } else { '' }
            $lv.BeginUpdate(); $lv.Items.Clear()
            if ($rm.Ok) {
                $list = @($rm.Data | Where-Object { $_ -and -not (Get-PelZtProp $_ 'deleted') })
                $st.Members = $list
                foreach ($m in $list) {
                    $mid = [string](Get-PelZtProp $m 'id')
                    $auth = [bool](Get-PelZtProp $m 'authorized')
                    $ips = (@(Get-PelZtProp $m 'ipAssignments') | Where-Object { $_ }) -join ', '
                    $seen = Get-PelZtProp $m 'lastSeen'
                    if ($null -eq $seen) { $seen = Get-PelZtProp $m 'lastOnline' }
                    $it = New-Object System.Windows.Forms.ListViewItem([string](Get-PelZtProp $m 'name'))
                    [void]$it.SubItems.Add($mid)
                    [void]$it.SubItems.Add($(if ($auth) { 'ja' } else { 'NEIN - wartet' }))
                    [void]$it.SubItems.Add($ips)
                    [void]$it.SubItems.Add((Format-PelZtSeen $seen))
                    [void]$it.SubItems.Add([string](Get-PelZtProp $m 'description'))
                    $it.Tag = $m
                    if (-not $auth) { $it.ForeColor = [System.Drawing.Color]::Orange }
                    [void]$lv.Items.Add($it)
                    if ($mid -eq $sel) { $it.Selected = $true }
                }
                $lblStatus.Text = "{0} Mitglied(er), davon {1} freigeschaltet  |  aktualisiert {2}" -f $list.Count, @($list | Where-Object { Get-PelZtProp $_ 'authorized' }).Count, (Get-Date -Format 'HH:mm:ss')
            } else { $lblStatus.Text = "Mitglieder nicht abrufbar: $($rm.Error)" }
            $lv.EndUpdate()
        } finally { $f.Cursor = [System.Windows.Forms.Cursors]::Default }
    }
    $selMember = {
        if ($lv.SelectedItems.Count -eq 0) { Show-PelMsg 'Bitte zuerst ein Mitglied in der Liste anklicken.' 'Mitglieder' ([System.Windows.Forms.MessageBoxIcon]::Information) $f; return $null }
        return $lv.SelectedItems[0].Tag
    }
    $update = {
        param($m, $body, [string]$okText)
        $mid = [string](Get-PelZtProp $m 'id')
        $r = Invoke-PelZtApi -Method 'POST' -Path "/api/v1/network/$NetId/member/$mid" -Body $body
        if ($r.Ok) { $lblStatus.Text = $okText; & $loadAll }
        else { Show-PelMsg "Änderung fehlgeschlagen:`n$($r.Error)" 'Mitglieder' ([System.Windows.Forms.MessageBoxIcon]::Error) $f }
    }

    $btnRef.Add_Click({ & $loadAll })
    $btnNetDel.Add_Click({ if (Remove-PelZtNetworkFlow $NetId $lblHead.Text $f) { $f.Close() } })
    $btnCopy.Add_Click({ try { [System.Windows.Forms.Clipboard]::SetText($NetId); $lblStatus.Text = "Netzwerk-ID $NetId kopiert." } catch { } })
    $btnJoin.Add_Click({ Invoke-PelZtJoin $NetId $f; & $loadAll })
    $btnWeb.Add_Click({ try { Start-Process ((Get-PelZtBaseUrl) + "/network/$NetId") } catch { } })
    $btnAuth.Add_Click({ $m = & $selMember; if ($m) { & $update $m @{ authorized = $true } "Freigeschaltet: $([string](Get-PelZtProp $m 'id'))" } })
    $btnDeauth.Add_Click({ $m = & $selMember; if ($m) { & $update $m @{ authorized = $false } "Gesperrt: $([string](Get-PelZtProp $m 'id'))" } })
    $renameAct = {
        $m = & $selMember
        if (-not $m) { return }
        $neu = Read-ZtText 'Umbenennen' 'Neuer Name für dieses Mitglied:' ([string](Get-PelZtProp $m 'name')) $f
        if ($null -ne $neu -and $neu -ne '') { & $update $m @{ name = $neu } "Umbenannt in '$neu'." }
    }
    $btnRename.Add_Click($renameAct)
    $lv.Add_DoubleClick($renameAct)
    $btnDesc.Add_Click({
        $m = & $selMember
        if (-not $m) { return }
        $neu = Read-ZtText 'Beschreibung' 'Beschreibung (z. B. "Gaming-PC von Max"):' ([string](Get-PelZtProp $m 'description')) $f
        if ($null -ne $neu) { & $update $m @{ description = $neu } 'Beschreibung gespeichert.' }
    })
    $btnDel.Add_Click({
        $m = & $selMember
        if (-not $m) { return }
        $mid = [string](Get-PelZtProp $m 'id')
        $ans = [System.Windows.Forms.MessageBox]::Show($f, "Mitglied $mid ($([string](Get-PelZtProp $m 'name'))) aus dem Netzwerk entfernen?`n`nZTNET legt es dabei nur beiseite - im Webpanel lässt es sich wiederherstellen.", 'Entfernen', [System.Windows.Forms.MessageBoxButtons]::YesNo, [System.Windows.Forms.MessageBoxIcon]::Question)
        if ($ans -ne [System.Windows.Forms.DialogResult]::Yes) { return }
        $r = Invoke-PelZtApi -Method 'DELETE' -Path "/api/v1/network/$NetId/member/$mid"
        if ($r.Ok) { $lblStatus.Text = "Entfernt: $mid"; & $loadAll }
        else { Show-PelMsg "Entfernen fehlgeschlagen:`n$($r.Error)" 'Entfernen' ([System.Windows.Forms.MessageBoxIcon]::Error) $f }
    })

    # alle 20 Sekunden neu laden (6 Anfragen/Minute - weit unter dem Limit von 50)
    $timer = New-Object System.Windows.Forms.Timer
    $timer.Interval = 20000
    $timer.Add_Tick({ if ($f.ContainsFocus) { & $loadAll } })
    $f.Add_Shown({ & $loadAll; $timer.Start() })
    $f.Add_FormClosing({ $timer.Stop(); $timer.Dispose() })
    if ($Owner) { [void]$f.ShowDialog($Owner) } else { [void]$f.ShowDialog() }
}

# ---- Fenster 2: meine Netzwerke --------------------------------------------------------
function Show-PelZtNetworkList($Owner) {
    $f = New-ZtForm 'Meine Netzwerke (ZTNET)' 720 480
    $lblTop = New-ZtLabel 'Deine Netzwerke - Klick auf ein Netzwerk öffnet die Verwaltung:' 12 12 680 20
    $lv = New-ZtList 12 36 520 360 @('Name', 'Netzwerk-ID', 'Mitglieder', 'Hier') @(170, 140, 80, 110)
    $btnNew  = New-ZtButton 'Neues Netzwerk' 544 36 150 34 $true
    $btnOpen = New-ZtButton 'Verwalten' 544 76 150 34
    $btnJoin = New-ZtButton 'Beitreten' 544 116 150 34
    $btnWeb  = New-ZtButton 'Webpanel' 544 156 150 34
    $btnRef  = New-ZtButton 'Aktualisieren' 544 196 150 34
    $lblStatus = New-ZtLabel '' 12 404 680 22
    $lblStatus.ForeColor = [System.Drawing.Color]::LightGray
    $f.Controls.AddRange(@($lblTop, $lv, $btnNew, $btnOpen, $btnJoin, $btnWeb, $btnRef, $lblStatus))
    Set-ZtAnchor $lv @('Top', 'Left', 'Right', 'Bottom')
    foreach ($c in @($btnNew, $btnOpen, $btnJoin, $btnWeb, $btnRef)) { Set-ZtAnchor $c @('Top', 'Right') }
    Set-ZtAnchor $lblStatus @('Left', 'Right', 'Bottom')

    $load = {
        $f.Cursor = [System.Windows.Forms.Cursors]::WaitCursor
        try {
            # welche Netzwerke hat dieser PC gerade? (für die Spalte "Hier")
            $lokal = @{}
            try { foreach ($n in @(Get-PelZtNetworkStatusList)) { if ($n.Id) { $lokal[[string]$n.Id] = [string]$n.Status } } } catch { }
            $r = Invoke-PelZtApi -Method 'GET' -Path '/api/v1/network'
            $lv.BeginUpdate(); $lv.Items.Clear()
            if ($r.Ok) {
                $nets = @($r.Data | Where-Object { $_ })
                foreach ($n in $nets) {
                    $id = [string](Get-PelZtProp $n 'nwid')
                    if (-not $id) { $id = [string](Get-PelZtProp $n 'id') }
                    $nm = [string](Get-PelZtProp $n 'name')
                    $it = New-Object System.Windows.Forms.ListViewItem($(if ($nm) { $nm } else { '(ohne Namen)' }))
                    [void]$it.SubItems.Add($id)
                    [void]$it.SubItems.Add([string](Get-PelZtProp $n 'memberCount'))
                    $hier = if ($lokal.ContainsKey($id)) { "verbunden ($($lokal[$id]))" } else { '-' }
                    [void]$it.SubItems.Add($hier)
                    if ($lokal.ContainsKey($id)) { $it.ForeColor = [System.Drawing.Color]::LightGreen }
                    $it.Tag = @{ Id = $id; Name = $nm }
                    [void]$lv.Items.Add($it)
                }
                $lblStatus.Text = "{0} Netzwerk(e)  |  {1}  |  aktualisiert {2}" -f $nets.Count, (Get-PelZtBaseUrl), (Get-Date -Format 'HH:mm:ss')
            } else { $lblStatus.Text = $r.Error -replace "`n", ' ' }
            $lv.EndUpdate()
        } finally { $f.Cursor = [System.Windows.Forms.Cursors]::Default }
    }
    $sel = {
        if ($lv.SelectedItems.Count -eq 0) { Show-PelMsg 'Bitte zuerst ein Netzwerk in der Liste anklicken.' 'Netzwerke' ([System.Windows.Forms.MessageBoxIcon]::Information) $f; return $null }
        return $lv.SelectedItems[0].Tag
    }
    $open = { $t = & $sel; if ($t) { Show-PelZtNetwork $t.Id $t.Name $f; & $load } }
    $lv.Add_MouseClick({ if ($lv.SelectedItems.Count -gt 0) { $t = $lv.SelectedItems[0].Tag; Show-PelZtNetwork $t.Id $t.Name $f; & $load } })
    $btnOpen.Add_Click($open)
    $btnRef.Add_Click({ & $load })
    $btnWeb.Add_Click({ try { Start-Process (Get-PelZtBaseUrl) } catch { } })
    $btnJoin.Add_Click({ $t = & $sel; if ($t) { Invoke-PelZtJoin $t.Id $f; & $load } })
    $btnNew.Add_Click({
        $name = Read-ZtText 'Neues Netzwerk' 'Name für das neue Netzwerk (z. B. "Clan LAN"):' '' $f
        if ($null -eq $name) { return }
        $body = @{}
        if ($name) { $body.name = $name }
        $r = Invoke-PelZtApi -Method 'POST' -Path '/api/v1/network' -Body $body
        if ($r.Ok) {
            $newId = ''
            if ($r.Data -is [string]) { $newId = $r.Data }
            else { foreach ($k in @('nwid', 'id', 'networkId')) { $v = Get-PelZtProp $r.Data $k; if ($v) { $newId = [string]$v; break } } }
            & $load
            $ans = [System.Windows.Forms.MessageBox]::Show($f, "Netzwerk erstellt$(if ($newId) { ": $newId" }).`n`nGib deinen Freunden diese ID - sie treten über Option 2 bei, du schaltest sie danach in der Mitgliederliste frei.`n`nMit diesem PC jetzt beitreten?", 'Netzwerk erstellt', [System.Windows.Forms.MessageBoxButtons]::YesNo, [System.Windows.Forms.MessageBoxIcon]::Information)
            if ($ans -eq [System.Windows.Forms.DialogResult]::Yes -and $newId) { Invoke-PelZtJoin $newId $f; & $load }
        } else { Show-PelMsg "Netzwerk konnte nicht erstellt werden:`n$($r.Error)" 'Neues Netzwerk' ([System.Windows.Forms.MessageBoxIcon]::Error) $f }
    })
    $f.Add_Shown({ & $load })
    if ($Owner) { [void]$f.ShowDialog($Owner) } else { [void]$f.ShowDialog() }
}
...
# ---- Fenster 1: Option 13 ------------------------------------------------------------
function Invoke-PelZtnetManager {
    $st = @{ LoggedIn = $false }
    $f = New-ZtForm 'Project Earth LAN - Eigene Netzwerke (ZTNET)' 760 560
    $f.FormBorderStyle = 'FixedDialog'; $f.MaximizeBox = $false

    $lblTitle = New-ZtLabel 'Eigene Netzwerke erstellen und verwalten' 20 14 700 26
    $lblTitle.Font = New-Object System.Drawing.Font('Segoe UI', 12, [System.Drawing.FontStyle]::Bold)
    $lblUrl = New-ZtLabel 'Panel-Adresse:' 20 50 110 22
    $txtUrl = New-ZtText 132 47 360
    $txtUrl.Text = Get-PelZtBaseUrl
    $btnUrl = New-ZtButton 'Standard' 500 45 110 28

    # Button 1: Account erstellen
    $btn1 = New-ZtButton '1. Account erstellen (Webpanel öffnen)' 20 90 700 40 $true
    $lbl1 = New-ZtLabel ("Öffnet die Registrierung im Webpanel. Dort legst du dein kostenloses Konto an. Voraussetzung: du bist mit dem Netzwerk 091f0945fc744570 (Account und Netzwerk Verwaltungs Server) verbunden - das richtet Option 1 ein.") 20 134 700 38
    $lbl1.ForeColor = [System.Drawing.Color]::LightGray

    # Token + Login
    $grp = New-Object System.Windows.Forms.GroupBox
    $grp.Text = 'Login mit deinem API-Token'
    $grp.ForeColor = [System.Drawing.Color]::White
    $grp.Location = New-Object System.Drawing.Point(20, 180)
    $grp.Size = New-Object System.Drawing.Size(700, 140)
    $lblTok = New-ZtLabel 'API-Token:' 14 30 80 22
    $lblSaved = New-ZtLabel '' 96 30 400 22
    $lblSaved.ForeColor = [System.Drawing.Color]::LightGreen
    $lblSaved.Visible = $false
    $txtTok = New-ZtText 96 27 400
    $txtTok.UseSystemPasswordChar = $true
    $chkShow = New-Object System.Windows.Forms.CheckBox
    $chkShow.Text = 'anzeigen'
    $chkShow.Location = New-Object System.Drawing.Point(504, 29)
    $chkShow.Size = New-Object System.Drawing.Size(90, 22)
    $btnLogin = New-ZtButton 'Login' 14 64 180 34 $true
    $btnLogout = New-ZtButton 'Abmelden' 202 64 140 34
    $btnTokPage = New-ZtButton 'Token-Seite öffnen' 350 64 200 34
    $lblLogin = New-ZtLabel 'Nicht angemeldet.' 14 106 670 24
    $lblLogin.ForeColor = [System.Drawing.Color]::LightGray
    $grp.Controls.AddRange(@($lblTok, $lblSaved, $txtTok, $chkShow, $btnLogin, $btnLogout, $btnTokPage, $lblLogin))

    # Button 2: Netzwerke
    $btn2 = New-ZtButton '2. Eigene Netzwerke erstellen und verwalten' 20 332 700 40
    $lbl2 = New-ZtLabel ("Hinweis: Dafür brauchst du deinen eigenen API-Token aus der Weboberfläche. Den erstellst du nach der Account-Erstellung im Panel unter deinem Benutzerkonto (Einstellungen -> Account -> API-Token), kopierst ihn und fügst ihn oben ein.`n`nDu gibst den Token nur einmal ein: Er wird auf diesem PC gespeichert (mit Windows verschlüsselt), beim nächsten Öffnen meldet dich der Manager automatisch an. Im Manager steckt kein Admin-Zugang - jeder sieht und verwaltet nur seine eigenen Netzwerke.") 20 376 700 110
    $lbl2.ForeColor = [System.Drawing.Color]::LightGray

    $f.Controls.AddRange(@($lblTitle, $lblUrl, $txtUrl, $btnUrl, $btn1, $lbl1, $grp, $btn2, $lbl2))

    $applyUrl = {
        $u = $txtUrl.Text.Trim().TrimEnd('/')
        if ($u -notmatch '^https?://[^\s/]+') { $u = $script:PelZtDefaultUrl; $txtUrl.Text = $u }
        if ($u -ne (Get-PelZtBaseUrl)) { Save-PelZtBaseUrl $u }
        return $u
    }
    # Gespeicherten Token anzeigen (nur die letzten 4 Zeichen) statt eines leeren Feldes
    $showSaved = {
        $s = Get-PelZtToken
        if ($s) {
            $end = if ($s.Length -gt 4) { $s.Substring($s.Length - 4) } else { '' }
            $lblSaved.Text = "Gespeichert (....$end) - wird automatisch verwendet"
            $lblSaved.Visible = $true; $txtTok.Visible = $false; $chkShow.Visible = $false
            $btnLogout.Text = 'Token löschen'
        } else {
            $lblSaved.Visible = $false; $txtTok.Visible = $true; $chkShow.Visible = $true
    ...
    $setState = {
        ...
        $st.LoggedIn = $ok
        $lblLogin.Text = $text
        $lblLogin.ForeColor = $(if ($ok) { [System.Drawing.Color]::LightGreen } else { [System.Drawing.Color]::LightGray })
        $btn2.BackColor = $(if ($ok) { [System.Drawing.Color]::FromArgb(0, 120, 215) } else { [System.Drawing.Color]::FromArgb(60, 60, 60) })
    }
    $check = {
        param([string]$tok)
        $u = & $applyUrl
        $r = Invoke-PelZtApi -Method 'GET' -Path '/api/v1/network' -Token $tok -BaseUrl $u
        if ($r.Ok) { & $setState $true ("Angemeldet - {0} eigene(s) Netzwerk(e) auf {1}" -f @($r.Data | Where-Object { $_ }).Count, $u) }
        elseif ($r.Status -eq 0) { & $setState $false ("Panel gerade nicht erreichbar - Token bleibt gespeichert, Login später erneut. " + (($r.Error -split "`n")[0])) }
        else { & $setState $false ("Login fehlgeschlagen: " + ($r.Error -replace "`n", ' ')) }
        return $r
    }

    $btnUrl.Add_Click({ $txtUrl.Text = $script:PelZtDefaultUrl; [void](& $applyUrl) })
    $txtUrl.Add_Leave({ [void](& $applyUrl) })
    $chkShow.Add_CheckedChanged({ $txtTok.UseSystemPasswordChar = -not $chkShow.Checked })
    $btn1.Add_Click({
        $u = & $applyUrl
        try { Start-Process ($u + '/auth/register') } catch { try { Start-Process $u } catch { } }
    })
    $btnTokPage.Add_Click({ $u = & $applyUrl; try { Start-Process ($u + '/user-settings/?tab=account') } catch { try { Start-Process $u } catch { } } })
    $btnLogin.Add_Click({
        $tok = $txtTok.Text.Trim()
        if (-not $tok) { $tok = Get-PelZtToken }
        if (-not $tok) { Show-PelMsg 'Bitte zuerst deinen API-Token aus der Weboberfläche einfügen.' 'Login' ([System.Windows.Forms.MessageBoxIcon]::Information) $f; return }
        $f.Cursor = [System.Windows.Forms.Cursors]::WaitCursor
        try {
            $r = & $check $tok
            # Speichern, wenn der Token gueltig ist - oder das Panel nur gerade nicht
            # erreichbar war (dann muss man ihn spaeter nicht noch einmal eintippen).
            # Ein abgelehnter Token (401) wird nicht gespeichert.
            if ($r.Ok -or $r.Status -eq 0) {
                try { Save-PelZtToken $tok; $txtTok.Text = ''; & $showSaved }
                catch { Show-PelMsg "Token konnte nicht gespeichert werden: $($_.Exception.Message)" 'Login' ([System.Windows.Forms.MessageBoxIcon]::Warning) $f }
            }
        } finally { $f.Cursor = [System.Windows.Forms.Cursors]::Default }
    })
    $btnLogout.Add_Click({
        ...
        $txtTok.Text = ''
        & $showSaved
        & $setState $false 'Abgemeldet - der gespeicherte Token wurde von diesem PC gelöscht. Neuen Token einfügen und Login klicken.'
    })
    $btn2.Add_Click({
        if (-not $st.LoggedIn -and (Get-PelZtToken)) {
            # gespeicherter Token vorhanden - einfach noch einmal versuchen
            $f.Cursor = [System.Windows.Forms.Cursors]::WaitCursor
            try { [void](& $check (Get-PelZtToken)) } finally { $f.Cursor = [System.Windows.Forms.Cursors]::Default }
        }
        if (-not $st.LoggedIn) {
            Show-PelMsg ("Bitte zuerst anmelden.`n`nDu brauchst deinen eigenen API-Token aus der Weboberfläche:`n1. Mit Button 1 einen Account erstellen`n2. Im Panel unter Einstellungen -> Account einen API-Token erstellen`n3. Den Token oben einfügen und auf Login klicken") 'Eigene Netzwerke' ([System.Windows.Forms.MessageBoxIcon]::Information) $f
            return
        }
        Show-PelZtNetworkList $f
    })
    $f.Add_Shown({
        & $showSaved
        $saved = Get-PelZtToken
        if ($saved) {
            $f.Cursor = [System.Windows.Forms.Cursors]::WaitCursor
            try { [void](& $check $saved) } finally { $f.Cursor = [System.Windows.Forms.Cursors]::Default }
        }
    })
    [void]$f.ShowDialog()
}
...
function Show-MainDashboard {
    ...
    $mainForm.Size = New-Object System.Drawing.Size(520, 1037)
    ...
    $buttons = @(
        @{ Text = "1. Project Earth LAN Installieren"; Action = { Show-PelVpnInstallChooser } },
        ...
        @{ Text = "13. Eigene Netzwerke erstellen & verwalten (ZTNET-Konto)"; Action = { Start-OptionWindow '13' { Invoke-PelZtnetManager } } }
    )
    ...
    # ÄNDERUNG: Vorher wurde hier ausschließlich der ZeroTier-CLI-Status abgefragt, auch
    # wenn der Nutzer z. B. Tailscale/WireGuard/Radmin VPN oder ein reines Heimnetzwerk
    # ausgewählt hatte - die Anzeige stand dann fälschlich immer auf "ZeroTier: Nicht
    # verbunden". Jetzt wird stattdessen der zentral gewählte Adapter (siehe
    # Get-PelSelectedAdapter, gilt für jedes VPN und für normales LAN/WLAN gleichermaßen)
    # geprüft: ist er noch aktiv, gilt der Manager als "Verbunden", unabhängig davon,
    # welche Technologie dahintersteckt.
    function Get-DashboardConnectionStatus {
        $r = [pscustomobject]@{ Connected = $false; Ip = ''; AdapterName = '' }
        $a = Get-PelSelectedAdapter
        if ($a) {
            $r.Connected = $true
            $r.Ip = $a.Ip
            $r.AdapterName = $a.Name
        }
        return $r
    }

    function Update-DashboardStatus {
        $conn = Get-DashboardConnectionStatus
        if ($conn.Connected) {
            $lblZt.Text = "Verbunden ($($conn.AdapterName))"
            $lblZt.ForeColor = [System.Drawing.Color]::LightGreen
            $lblIp.Text = "Eigene IP: $($conn.Ip)"
        } else {
            $lblZt.Text = "Nicht verbunden"
    ...
    $btnJoinPlayer.Add_Click({
        ...
        $tIp = $ip
        $tJoin = 0
        # Bei einem Server-Prozess den Spiel-Client starten, nicht den Server
        $joinName = ([string]$m.Game) -replace '\s*\(Server\)$', ''
        $joinHint = [string]$m.GameExe
        if ($joinHint -match '(?i)server|dedicated|^srcds|^ucc\.exe$|^hlds') { $joinHint = '' }
        ...
        if ($tJoin -gt 0) {
            $res = Invoke-PelJoinGame -Ip $tIp -JoinPort $tJoin -GameName $joinName -ExeHint $joinHint -ParentForm $mainForm
        } else {
            ...
            $res = Invoke-PelJoinGame -Ip $tIp -GameName $joinName -ExeHint $joinHint -ParentForm $mainForm -LaunchOnly
        }
    ...
    $statPoll.Add_Tick({
        ...
        try {
            ...
            if ($statTx -and ($script:PelStatusHbTick % 16 -eq 0)) {
                try {
                    $ztNow = Get-DashboardConnectionStatus
                ...
                try {
                    while ($statRx.Available -gt 0) {
                        ...
                        if ($txt.StartsWith('PESTAT1|')) {
                            ...
                            # Ein PC mit mehreren Netzwerkwegen (z. B. ZeroTier + WLAN) sendet seinen
                            # Status über jeden Weg - sonst stünde er doppelt in der Liste. Gleicher Name
                            # + gleiche gemeldete IP = derselbe Manager; bevorzugt wird der Eintrag, der
                            # direkt von seiner gemeldeten IP kommt.
                            $dupKey = $null
                            foreach ($ok in @($script:PelOtherManagers.Keys)) {
                                $om = $script:PelOtherManagers[$ok]
                                if ($ok -ne $srcIp -and $om.Name -eq $mgr.Name -and $om.Ip -and $om.Ip -eq $mgr.Ip) { $dupKey = $ok; break }
                            }
                            if ($dupKey) {
                                if ($script:PelOtherManagers[$dupKey].SrcIp -eq $mgr.Ip -and $srcIp -ne $mgr.Ip) {
                                    # vorhandenen (direkten) Eintrag nur auffrischen
                                    $mgr.SrcIp = $script:PelOtherManagers[$dupKey].SrcIp
                                    $script:PelOtherManagers[$dupKey] = $mgr
                                    continue
                                }
                                $script:PelOtherManagers.Remove($dupKey)
                            }
